...
library(picante)
phylo<-read.tree('Poyang202208_16S.tree')
comun<-read.table('Poyang202208_16Sotu.txt',sep='\t',row.names=1,header=T)
comun=t(comun)
...
identical(colnames(match.phylo.comun$data),colnames(beta.mntd.weighted))
identical(colnames(match.phylo.comun$data),rownames(beta.mntd.weighted))
...
library(ecodist)
RC.bray=raup_crick_abundance(comun,reps=999,plot_names_in_col1=F)
RC.mat=as.matrix(RC.bray)
write.csv(RC.mat,'RC_bray.csv',quote=F)